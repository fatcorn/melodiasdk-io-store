...
func TestMultiversionItemInsert(t *testing.T) {
	...
	for i := 0; i < 1000000; i++ {
		go func(i int) {
			defer wg.Done()
			one := []byte("one")
			mvItem.Set(i, i%20, one)
		}(i)
	}

	wg.Wait()
	t2 := time.Now()
	println("Insert time", "start time", now.String(), "end time", t2.String(), "sub", t2.Sub(now).Milliseconds())
}
